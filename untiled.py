...
def podsumowanie(): #* Wyświetla ekran podsumowania
    ...
    Label(top, text="Znalezione błędy: " + str(licznikP), font=('Calibri 14 bold')).place(relx=0.55, rely=0.10, anchor=W)
    Button(top, text="Zatwierdź", padding=2, command=zatwierdzanie_nr).place(relx=0.65, rely=0.25)

    Button(top, text="Importuj nowy", style='b1.TButton', command=close).place(relx=0.15, rely=0.58)
    Button(top, text="Zamknij", style='b2.TButton', command=exit).place(relx=0.56, rely=0.58)
...
def submit():
    if czyPoprawic.get() == 1: #* Jeżeli opcja popraw odstępy jest zaznaczona (tymcz. wyłączone)
        ...
        with open(r'C:/Users/DawidZygmunt/OneDrive - Snap Outdoor Sp. z o.o/Dokumenty/output/zapis.txt', 'w', newline='') as export:
            global licznikP
            for x in tablica:
            ...
            podsumowanie()
        error_popup("Funkcja wyłączona!" ,"Funkcja wyłączona z powodu możliwych blędów")
